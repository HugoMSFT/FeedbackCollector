...
def clean_feedback_text(text: str) -> str:
    ...
    text = re.sub(
        r"[a-zA-Z]+:\s*p\.[a-zA-Z]+,?\s*li\.[a-zA-Z]+,?\s*div\.[a-zA-Z]+\s*\{?", "", text, flags=re.IGNORECASE
    )
...
def detect_audience(text: str, source: str = "", scenario: str = "", organization: str = "") -> str:
    ...
    if scenario.lower() == "partner":
        ...
        if any(
            isv_term in text_lower
            for isv_term in ["isv", "independent software vendor", "multi-tenant", "tenant", "saas"]
        ):
    ...
    if any(
        term in text_lower for term in ["multi-tenant", "saas", "software as a service", "independent software vendor"]
    ):
        audience_scores["ISV"] += 3
...
def get_category_statistics(feedback_items: list) -> dict:
    ...
    stats["audience_category_matrix"] = {
        audience: dict(categories) for audience, categories in stats["audience_category_matrix"].items()
    }
    stats["domain_audience_matrix"] = {
        domain: dict(audiences) for domain, audiences in stats["domain_audience_matrix"].items()
    }
    ...
    for item in feedback_items:
        ...
        if audience == "Developer" and enhanced["priority"] in ["high", "critical"]:
            trends["top_developer_requests"].append(
                {"feedback": item.get("Feedback_Gist", ""), "subcategory": subcategory, "priority": priority}
            )
        ...
        if enhanced["priority"] == "critical":
            trends["critical_issues"].append(
                {"feedback": item.get("Feedback_Gist", ""), "audience": audience, "subcategory": subcategory}
            )
...
def analyze_sentiment(text: str) -> dict:
    ...
    """
    if not text or not isinstance(text, str):
        return {"polarity": 0.0, "subjectivity": 0.0, "label": "Neutral", "confidence": "Low"}
...
def analyze_repeating_requests(feedback_items: list) -> dict:
    ...
    analysis = {
        ...
        "top_repeating_requests": [
            {
                "summary": (
                    cluster["primary_item"].get("Feedback_Gist", "")[:100] + "..."
                    if len(cluster["primary_item"].get("Feedback_Gist", "")) > 100
                    else cluster["primary_item"].get("Feedback_Gist", "")
                ),
                "count": cluster["count"],
    ...
    logger.info(
        f"Repeating request analysis complete: {len(actual_repeating_clusters)} clusters found, {repetition_rate}% repetition rate, {unique_requests} unique requests"
    )
