...
def _prepare_pyspark_payload(data_list: list, table_name: str, write_mode: str) -> str:
    ...
    try:
        ...
        logger.debug(f"JSON string length: {len(data_as_json_string)} chars")
    ...
    logger.debug(f"Python literal string length: {len(data_as_python_literal_string)} chars")
...
def _test_fabric_token(token: str, livy_endpoint: str, spark_conf: dict = None) -> str:
    ...
    session_payload = {
        "name": "FeedbackCollectorTokenTest",
        "kind": "pyspark",
        "conf": spark_conf or DEFAULT_SPARK_CONF,
    }
    try:
...
def _start_livy_session(token: str, livy_endpoint: str, spark_conf: dict = None) -> str:
    ...
    try:
        ...
        session_state = session_data.get("state")
        if session_id is None:
            logger.error(f"Livy session ID not found: {session_data}")
            return None
        ...
        for _ in range(30):
            ...
            if current_state == "idle":
                logger.info(f"Session {session_id} is idle.")
                return session_id
        ...
        logger.error(f"RequestException starting Livy session: {e}")
        return None
    except Exception as e:
        logger.error(f"Unexpected error starting Livy session: {e}", exc_info=True)
        return None


def _submit_spark_statement(token: str, livy_endpoint: str, session_id: int, spark_code: str) -> str:
    if not spark_code:
        logger.error("No Spark code to submit.")
        return None
    ...
    try:
        ...
        response = requests.post(
            statement_url, headers=headers, json=payload, timeout=180
        )  # Increased timeout for potentially larger code
        response.raise_for_status()
        statement_id = response.json().get("id")
        if statement_id is None:
            logger.error(f"Statement ID not found: {response.json()}")
            return None
        ...
        logger.error(f"RequestException submitting Spark statement: {e}")
        return None
    except Exception as e:
        logger.error(f"Unexpected error submitting Spark statement: {e}", exc_info=True)
        return None
        ...
        try:
            ...
            if state == "available":
                ...
                if output and output.get("status") == "ok":
                    ...
                    if spark_stdout:
                        logger.info(f"Spark stdout (first 1000 chars):\n{spark_stdout[:1000]}")
                    # Check for application-level errors in stdout even on "ok" status
                    if (
                        "FATAL: Error decoding JSON data in Spark" in spark_stdout
                        or "Error during DataFrame creation or writing to table" in spark_stdout
                    ):
                        logger.error(
                            f"Spark job reported an application error in its output for statement {statement_id}."
                        )
                        if spark_stdout:
                            logger.error(f"Full Spark stdout for statement {statement_id}:\n{spark_stdout}")
                        return False
                    return True
                elif output and output.get("status") == "error":
                    logger.error(
                        f"Statement {statement_id} error. Name: {output.get('ename')}, Value: {output.get('evalue')}\nTraceback: {''.join(output.get('traceback', []))}"
                    )
                    if spark_stdout:
                        logger.error(f"Spark stdout on error for statement {statement_id}:\n{spark_stdout}")
                    return False
                else:
                    logger.warning(f"Statement {statement_id} available, unexpected output: {output}")
                    if spark_stdout:
                        logger.warning(f"Spark stdout for unexpected output:\n{spark_stdout}")
                ...
                if spark_stdout:
                    logger.error(f"Spark stdout on failure/cancellation:\n{spark_stdout}")
                return False
        except requests.exceptions.HTTPError as e:
            logger.error(
                f"HTTP error polling statement: {e}. Status: {e.response.status_code if e.response else 'N/A'}. Text: {e.response.text if e.response else 'N/A'}"
            )
            ...
            logger.error(f"RequestException polling statement: {e}")
            time.sleep(20)
        except Exception as e:
            logger.error(f"Unexpected error polling statement: {e}", exc_info=True)
            return False
...
def _close_livy_session(token: str, livy_endpoint: str, session_id: int) -> bool:
    ...
    try:
        ...
        logger.error(
            f"Error closing Livy session {session_id}: {e}" + (f"\nResponse: {e.response.text}" if e.response else "")
        )
        return False
    except Exception as e:
        logger.error(f"Unexpected error closing Livy session {session_id}: {e}", exc_info=True)
        return False


def write_data_to_fabric(token: str, data_to_write: list) -> bool:
    if not token:
        logger.error("Fabric access token not provided.")
        return False
    if not data_to_write:
        logger.info("No data provided to write to Fabric.")
        return True
    ...
    try:
        ...
        if not spark_code:
            logger.info("PySpark payload is empty. Skipping submission.")
            return True
        ...
        if not success:
            logger.error(f"Spark statement {statement_id} processing failed.")
            return False
    ...
    if manual_test_token == "YOUR_MANUALLY_EXTRACTED_FABRIC_TOKEN":
        ...
        for i in range(3):
            ...
            if "FeedbackText" in item:
                item["FeedbackText"] = (
                    f"Sample text for item {i+1} with newline\n and unicode éà here. Also 'single quotes' and \"double quotes\"."
                )
            item["Created"] = time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())
            sample_data_list.append(item)

        print(
            f"Sample data to write ({len(sample_data_list)} items): {json.dumps(sample_data_list, indent=2, ensure_ascii=False)}"
        )
        ...
        if success:
            print("Test write_data_to_fabric reported SUCCESS.")
        else:
            print("Test write_data_to_fabric reported FAILURE.")
