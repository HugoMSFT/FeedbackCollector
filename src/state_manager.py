...
def update_feedback_domain(feedback_id: str, new_domain: str, user: str) -> Dict[str, Any]:
    ...
    return {"Feedback_ID": feedback_id, "Primary_Domain": new_domain, "Last_Updated": now, "Updated_By": user}
...
def format_state_for_display(state: str) -> Dict[str, str]:
    ...
    return {"name": state_info["name"], "color": state_info["color"], "description": state_info.get("description", "")}
...
def update_feedback_field_in_sql(feedback_id: str, field_name: str, new_value: str, bearer_token: str) -> bool:
    ...
    try:
        ...
        if field_name in ["Primary_Domain", "Category", "Subcategory", "Feature_Area"]:
            ...
            state_update_query = (
                f"UPDATE FeedbackState SET {field_name} = ?, Last_Updated = ?, Updated_By = ? WHERE Feedback_ID = ?"
            )
            ...
            logger.info(
                f"✅ Updated {field_name} in both Feedback table ({feedback_rows_updated} rows) and FeedbackState table"
            )
        ...
        try:
            ...
            logger.error(f"Error in state loading: {e}")
        return False
