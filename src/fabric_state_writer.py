...
class FabricStateWriter:
    ...
    def write_state_changes(self, state_changes: List[Dict[str, Any]]) -> bool:
        ...
        try:
            logger.warning(
                f"🔥 FABRIC WRITE INITIATED - Writing {len(state_changes)} state changes to Fabric Lakehouse"
            )
    ...
    def _write_single_state_change(self, change: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            update_payload = {"feedback_id": feedback_id, "timestamp": datetime.now().isoformat(), "changes": {}}
...
""",
                ...
                response = requests.post(
                    f"{self.livy_url}/sessions", headers=self.headers, json=livy_payload, timeout=30
                )
    ...
    def create_state_history_record(
        self, feedback_id: str, old_state: str, new_state: str, notes: str, updated_by: str
    ) -> bool:
        """
